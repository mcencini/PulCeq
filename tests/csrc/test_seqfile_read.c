...
MU_TEST(test_seqFile_getBlock_valid) {
    ...
    printf("[DEBUG] Calling getBlock(seq, 0, 1)...\n");
...
MU_TEST(test_seqFile_getBlock_invalid) {
    printf("[DEBUG] Entering test_seqFile_getBlock_invalid\n");
    SeqFile* seq;
    SeqBlock* block;
    printf("[DEBUG] Calling seqFile...\n");
    seq = seqFile("../../tests/expected_output/seq6.seq");
    printf("[DEBUG] seq pointer: %p\n", (void*)seq);
    mu_assert(seq != NULL, "Failed to open seq6.seq");
    printf("[DEBUG] Calling getBlock(seq, -1, 1)...\n");
...
MU_TEST(test_seqFile_block_values) {
    ...
    printf("[DEBUG] Calling getBlock(seq, 3, 1)...\n");
